...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        t, t_lengths = t.cuda(rank, non_blocking=True), t_lengths.cuda(
            rank, non_blocking=True
        )
        # s, s_lengths = s.cuda(rank, non_blocking=True), s_lengths.cuda(
        #     rank, non_blocking=True
        # )
        # p, p_lengths = p.cuda(rank, non_blocking=True), p_lengths.cuda(
        #     rank, non_blocking=True
        # )
        token2word = token2word.cuda(rank, non_blocking=True)
        word2phone = word2phone.cuda(rank, non_blocking=True)
        speakers = speakers.cuda(rank, non_blocking=True)
        # import pdb; pdb.set_trace()
        flag = global_step > hps.train.bert_finetune 
        if flag == 1 and global_step == hps.train.bert_finetune+1:
            print('start to finetune bert')
        with autocast(enabled=hps.train.fp16_run):
            (
                ...
                (hidden_x, logw, logw_),
            ) = net_g(x, x_lengths, spec, spec_lengths, t, t_lengths, token2word, word2phone, flag, speakers)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (
            ...
            t, t_lengths = t.cuda(0), t_lengths.cuda(0)
            s, s_lengths = s.cuda(0), s_lengths.cuda(0)
            p, p_lengths = p.cuda(0), p_lengths.cuda(0)
            token2word = token2word.cuda(0)
            word2phone = word2phone.cuda(0)
            ...
            speakers = speakers[:1]
            t = t[:1]
            t_lengths = t_lengths[:1]
            s = s[:1]
            s_lengths = s_lengths[:1]
            p = p[:1]
            p_lengths = p_lengths[:1]
            word2phone = word2phone[:1]
            token2word = token2word[:1]
            break
        flag = 0
        y_hat, attn, mask, *_ = generator.module.infer(
            x, x_lengths, spec, spec_lengths, t, t_lengths, token2word, word2phone, flag, speakers, max_len=1000
        )
