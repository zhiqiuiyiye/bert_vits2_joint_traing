...
bert_model = BertModel.from_pretrained("/data_02/chuyinxue/research/Bert_styletts/bert-base-multilingual-cased")
...
def avg_bert(output,token2word):
    # a = torch.zeros(output.shape[0],11,output.shape[1])
    # import pdb;pdb.set_trace()
    # for i in range(len(length)):
    #     sum=0
    #     for j in range(len(length[i])):
    #         a[i][j][sum:sum+length[i][j]]=1/(length[i][j]+(1e-9))
    #         sum += length[i][j]
    # import pdb;pdb.set_trace()
    avg = torch.bmm(token2word,output)

    return avg

def avg_bert1(output,word2phone):
    # a = torch.zeros(output.shape[0],output.shape[1],13)
    output = output.permute(0,2,1)
    # import pdb;pdb.set_trace()
    # for i in range(len(length)):
    #     sum=0
    #     for j in range(len(length[i])):
    #         a[i][j][sum:sum+length[i][j]]=1
    #         sum += length[i][j]
    # import pdb;pdb.set_trace()
    avg = torch.bmm(output,word2phone)
...
class TextEncoder(nn.Module):
    def __init__(
        ...
        self.bert_proj = nn.Linear(768, hidden_channels)

    def forward(self, x, x_lengths, t, t_lengths, token2word, word2phone, flag, g=None):
        x = self.emb(x) * math.sqrt(self.hidden_channels)
        if flag == 0:
            with torch.no_grad():
                t_mask = torch.unsqueeze(commons.sequence_mask(t_lengths, t.size(1)), 1).to(t.dtype)
                bert_out = self.bert(t,attention_mask=t_mask)[0].detach()
                # print('bert_no_finetune')
        else:
            t_mask = torch.unsqueeze(commons.sequence_mask(t_lengths, t.size(1)), 1).to(t.dtype)
            for params in self.bert.parameters():
                params.requires_grad = False
            for params in self.bert.encoder.layer[-2:].parameters():
                params.requires_grad = True
            # print('bert_finetune')
            bert_out = self.bert(t,attention_mask=t_mask)[0]
        # import pdb;pdb.set_trace()
        bert_out = avg_bert(bert_out,token2word)
        # import pdb;pdb.set_trace()
        # print('bert_out.shape',bert_out.shape)
        # print('bert_out[0,:,0]',bert_out[0,:,0])
        bert_out = avg_bert1(bert_out,word2phone)
        # import pdb;pdb.set_trace()
        # print('bert_out.shape',bert_out.shape)
        # print('bert_out[0,:,0]',bert_out[0,:,0])
        x = self.bert_proj(bert_out) + x
        # print(x.shape)
...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, x, x_lengths, y, y_lengths, t, t_lengths, token2word, word2phone, flag, sid=None):
        ...
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths,t, t_lengths, token2word, word2phone, flag, g=g)
    ...
    def infer(
        ...
        #     g = None
        # import pdb;pdb.set_trace()
        refy_mask = torch.unsqueeze(commons.sequence_mask(refy_lengths, refy.size(2)), 1).to(
            refy.dtype
        )
        g = self.ref_enc(refy * refy_mask, refy_mask)
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, t, t_lengths, token2word, word2phone, flag, g=g)
        if self.use_sdp:
